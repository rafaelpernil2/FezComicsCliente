...
export class UserProvider implements HttpMethodsInterface {
    ...
    getUserByToken(token: string): Observable<User>{
        let options = new RequestOptions({ headers:this.obtainHeaders(),withCredentials: true});
        return this.http.get("http://fezcomic.jelastic.cloudhosted.es/B3servidorREST/webresources/app.entities.user/userbytoken/{token}" + token).pipe(map(response => { return response.json() }));
    }

    getRol(id: string): Observable<User>{
        let options = new RequestOptions({ headers:this.obtainHeaders(),withCredentials: true});
        return this.http.get("http://fezcomic.jelastic.cloudhosted.es/B3servidorREST/webresources/app.entities.user/{id}" + id).pipe(map(response => { return response.json() }));
    }
