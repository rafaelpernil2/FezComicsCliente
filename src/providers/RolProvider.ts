...
export class RolProvider implements HttpMethodsInterface {
    ...
    all(): Observable<Rol[]> {
        ...
        return this.http.get<Rol[]>(this.basicUrl);
    }

    get(id: number): Observable<Rol> {
        const options = { headers: this.obtainHeaders(), withCredentials: true};
        return this.http.get<Rol>(this.basicUrl + '/' + id);
    }

    put(id: number, rol: Rol): Observable<Rol> {
        const options = { headers: this.obtainHeaders(), withCredentials: true};
        return this.http.put<Rol>(this.basicUrl + '/' + id + '/', rol);
    }
    post(rol: Rol): Observable<Rol> {
        const options = { headers: this.obtainHeaders(), withCredentials: true};
        return this.http.post<Rol>(this.basicUrl, rol);
    }
    delete(id: number): Observable<Rol> {
        const options = { headers: this.obtainHeaders(), withCredentials: true};
        return this.http.delete<Rol>(this.basicUrl + '/' + id);
    }

    count(): Observable<Rol> {
        const options = { headers: this.obtainHeaders(), withCredentials: true};
        return this.http.get<Rol>(this.basicUrl + '/' + 'count');
    }
}
