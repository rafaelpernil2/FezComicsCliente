...
import {blobToBase64String, binaryStringToBlob, blobToBinaryString} from 'blob-util';
...
export class ComicPage implements OnInit {
  ...
  onSubmit(){
    //this.isFile ? this.uploader.uploadAll() : 
    this.uploadComic();
  }
1
  ...
  uploaderController() {
    this.uploader.onAfterAddingFile = (fileItem : FileItem)=>{
      ...
      if(!fileItem.file.type.includes("image")){
        ...
        this.isFile = false;
        // this.filePath = environment.serverUrl+'/containers/public/download/'+this.me.photo;
      } else {
        
        
        
        blobToBase64String(fileItem._file.slice()).then(function (result) {
          console.log("ARCHIVO SUBIDO" , result);
          
          
          
          console.log("THIS COMIC FOTO");
        }).catch(function (err) {
          // error
        });
        //this.foto = this.sanitizer.bypassSecurityTrustUrl("data:image/jpeg;base64, " );
